...
export default function StartPage() {
  ...
  return (
    <div
      className="min-h-screen text-gray-900 font-sans overflow-x-hidden"
      style={{ background: "#F5F3FF" }}
    >
      {/* ── MINIMAL HEADER ── */}
      <SuperNavbar
        logoSrc="/logo.png"
        links={TOOL_NAV_LINKS}
        cta={{
          label: "Start Free →",
      ...
      <section
        id="pricing"
        className="py-20 px-5 md:px-12 border-t border-violet-100"
        style={{ background: "#F5F3FF" }}
      >
            ...
            {PRICING_TIERS.map((tier) => (
              ...
              >
                ...
                <div className="flex items-baseline gap-1 mb-6">
                  <span
                    className={`text-4xl font-black ${tier.highlight ? "text-white" : ""}`}
                ...
                <ul className="space-y-3 mb-8">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 text-sm">
                      <CheckCircle2
                        className={`w-4 h-4 flex-shrink-0 mt-0.5 ${tier.highlight ? "text-emerald-300" : ""}`}
        ...
        sections={[
          {
            title: "Tools",
            links: [
              { label: "Social Media Manager", href: "/social" },
              { label: "Ads Center", href: "/ads" },
              { label: "Business Discovery Directory", href: "/directory" },
              { label: "Brand & Digital Home", href: "/brand" },
              { label: "Investor Readiness Suite", href: "/investor" },
              { label: "Project Manager", href: "/projects" },
              { label: "Boldmind Fitness Center", href: "/fitness" },
              { label: "Marketing Automation", href: "/marketing" },
              { label: "CRM & Client Management", href: "/crm" },
              { label: "AI Business Agent", href: "/agent" },
              { label: "Business Intelligence Suite", href: "/intelligence" },
              { label: "HR & Payroll", href: "/hr" },
              { label: "Boldmind Marketplace", href: "/marketplace" },
            ],
          },
          {
