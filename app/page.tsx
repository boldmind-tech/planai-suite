...
const TESTIMONIALS = [
  ...
  },
  {
    ...
    quote: 'ViralKit schedules content for all my clients in one session. What used to take me 8 hours every week now takes 45 minutes.',
    product: 'ViralKit',
  ...
  },
  {
    ...
    quote: 'Credibility Hubs built me a portfolio that got me into a corporate supplier conversation. Worth every kobo of the ₦5k.',
...
function TagBadge({ tag }: { tag: 'LIVE' | 'BUILDING' | 'PLANNED' }) {
  ...
  return (
    <span className={`text-[9px] font-black tracking-[0.18em] px-2 py-0.5 rounded-full ${classes[tag]}`}>
      {tag}
    </span>
  );
}
...
export default function PlanAILanding() {
  ...
  return (
    <div className="min-h-screen bg-violet-50 text-gray-900 font-sans overflow-x-hidden">
      ...
      <SuperNavbar
        logoSrc="/logo.png"
        cta={{ label: 'Start Free →', href: 'https://boldmind.ng/register?redirect=https://planai.boldmind.ng' }}
        sticky={true}
      />
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <h1 className="text-5xl md:text-7xl font-black tracking-tight text-gray-900 mb-6 leading-[1.05]">
              One Suite.<br />
              <span className="text-violet-700">Every Tool</span> Your<br />
              Nigerian Business Needs.
            </h1>

            <p className="text-lg md:text-xl text-gray-500 max-w-xl mx-auto leading-relaxed mb-10">
              AI Receptionist, Digital Storefronts, Branding, ViralKit, Financial Forecasting & more — one login, one price, built for Nigeria.
            </p>
            ...
            <div className="flex flex-wrap items-center justify-center gap-10 md:gap-16">
              {[
                { n: `${businesses}+`, label: 'Businesses Running', color: 'text-violet-700' },
                { n: `₦${revenue}M+`, label: 'Monthly Revenue Processed', color: 'text-emerald-600' },
                { n: 'Built', label: 'in Lagos, Nigeria', color: 'text-sky-700' },
              ].map(({ n, label, color }) => (
                <div key={label} className="text-center">
                  <div className={`text-3xl font-black leading-none mb-1 ${color}`}>{n}</div>
      ...
      <section className="py-20 px-6 md:px-12 bg-violet-50 border-t border-violet-100">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-4xl md:text-5xl font-black text-gray-900 mb-4">Real Businesses. Real Results.</h2>
            <p className="text-gray-400 text-lg">650+ Nigerian businesses already running on PlanAI.</p>
          </div>

          <div className="grid md:grid-cols-3 gap-5">
            {TESTIMONIALS.map((t) => (
              <div key={t.name} className="bg-white rounded-2xl p-7 border border-violet-100">
                <div className="flex gap-0.5 mb-4">
                  {[1,2,3,4,5].map(i => <Star key={i} className="w-4 h-4 fill-amber-400 text-amber-400" />)}
                </div>
                <p className="text-sm text-gray-500 leading-relaxed mb-6 font-light italic">"{t.quote}"</p>
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-right">
                    <div className="text-[10px] text-violet-600 font-bold uppercase tracking-wider">{t.product}</div>
                    <div className="text-sm text-emerald-600 font-black mt-0.5">{t.revenue}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      {/* PRICING */}
      <section id="pricing" className="py-20 px-6 md:px-12 bg-white border-t border-violet-100">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            <h2 className="text-4xl md:text-5xl font-black text-gray-900 mb-4">Simple Pricing</h2>
            <p className="text-gray-400 text-lg">No hidden fees. No dollar pricing. 100% Naira-first.</p>
          </div>
            ...
            {PRICING_TIERS.map((tier) => (
              ...
              >
                ...
                <div className="flex items-baseline gap-1 mb-6">
                  <span className={`text-4xl font-black ${tier.highlight ? 'text-white' : 'text-violet-700'}`}>
                    {tier.price}
                  </span>
                ...
                <ul className="space-y-3 mb-8">
                  {tier.features.map((f) => (
                    <li key={f} className="flex items-start gap-2.5 text-sm">
                      <CheckCircle2 className={`w-4 h-4 flex-shrink-0 mt-0.5 ${tier.highlight ? 'text-emerald-300' : 'text-emerald-500'}`} />
      ...
      {/* ECOSYSTEM */}
      <section id="ecosystem" className="py-20 px-6 md:px-12 bg-violet-50 border-t border-violet-100">
        <div className="max-w-6xl mx-auto">
          <div className="text-center mb-14">
            <p className="text-violet-600 text-[11px] font-black tracking-[0.25em] uppercase mb-3">BoldMind Ecosystem</p>
            <h2 className="text-4xl md:text-5xl font-black text-gray-900 mb-4">PlanAI Is One Door of Four</h2>
            <p className="text-gray-400 text-lg max-w-md mx-auto">
              One BoldMind account unlocks the entire ecosystem. From gist to conviction to skills to tools.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-5 mb-8">
            {ECOSYSTEM_PILLARS.map((p) => (
              ...
              >
                ...
                <h3 className={`text-lg font-black mb-1 ${p.colorClass}`}>{p.name}</h3>
                <p className={`text-xs font-semibold mb-3 opacity-70 ${p.colorClass}`}>{p.tagline}</p>
                <p className="text-sm text-gray-500 leading-relaxed font-light mb-4">{p.desc}</p>
                <span className={`text-sm font-bold ${p.colorClass}`}>Explore →</span>
              </a>
            ))}
          </div>
          ...
          <div className="bg-white rounded-2xl border border-violet-100 p-6 text-center">
            <p className="text-sm text-gray-500 mb-3 max-w-2xl mx-auto">
              One BoldMind account. All four pillars. The flywheel: AmeboGist brings readers → VillageCircle builds conviction → EduCenter activates skills → PlanAI enables building.
            </p>
            <a
      ...
      <section className="py-20 px-6 md:px-12 bg-white border-t border-violet-100">
        <div className="max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
            {[
              { q: 'Can I use PlanAI on mobile?', a: 'Yes. PlanAI is a PWA — works on any smartphone browser without downloading an app. Optimised for Android and iOS.' },
              { q: 'Do you support Paystack?', a: 'Yes. Digital Storefronts, Financial Forecasting, and Analytics Dashboard all integrate natively with Paystack.' },
              { q: 'What\'s the difference between Starter and Pro?', a: 'Starter gives you 1 tool. Pro unlocks the full suite — all 10 tools, unlimited contacts, team seats, and priority support.' },
              { q: 'Is there a free trial?', a: 'Yes — 7 days free, no credit card required. Every plan starts with a free trial.' },
              { q: 'Can I pay in dollars?', a: 'All plans are priced in Naira for now. A diaspora tier is coming soon.' },
              { q: 'Is this compliant with Nigerian regulations?', a: 'Yes. We comply with NDPA data rules, and our e-invoicing module is built with FIRS guidelines in mind.' },
            ].map(({ q, a }, i) => (
              <details key={i} className="bg-slate-50 border border-slate-100 rounded-2xl p-6 group open:shadow-sm">
      ...
      <section className="py-24 px-6 bg-gradient-to-br from-violet-800 via-violet-700 to-violet-900 text-center">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-violet-200 text-lg mb-10 leading-relaxed">
            Every day you wait is another DM that goes unanswered, another competitor capturing your leads, another week without a real online store.
          </p>
